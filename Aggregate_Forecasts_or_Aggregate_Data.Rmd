...
(Boostingeval <-c(classificationMetrics(test.data[,"marks"],boost.pred,posClass = "1"),AUC=roc.curve(test.data[,"marks"],boost.pred,FALSE)$auc,classificationMetrics(test.data[,"marks"],boost.pred,posClass = 1,"totU",cbM)))
```
...
(NNeval <- c(classificationMetrics(test.data[,"marks"],nn.pred,posClass = "TA"),AUC=roc.curve(test.data[,"marks"],nn.pred,FALSE)$auc,classificationMetrics(test.data[,"marks"],nn.pred,posClass = "TA","totU",cbM)))
```

# Evaluation table
```{r}
(EvalTable <- rbind(Nulleval,LReval,LDAeval,QDAeval,KNNeval,Lassoeval,GAMeval,RFeval,Boostingeval,SVMeval,NNeval))
```
...
svm.pred <- as.factor(svm.pred)
  ...
  table_all <- cbind(ideal_matrix[-train,1:3],class1[,4],class2[,4],class3[,4],class4[,4],class5[,4],class6[,4],class7[,4],class8[,4],class9[,4],class10[,4],class11[,4])
  colnames(table_all) <- c("AF","AD","Ideal","LR","LDA","QDA","KNN","Lasso","GAM","RF","Boosting","SVM","Null", "NNet")
...
colnames(table_all) <- c("AF","AD","Ideal","LR","LDA","QDA","KNN","Lasso","GAM","RF","Boosting","SVM","Null","Neural Network")
...
table_CB <- matrix(nrow = 1, c(AF_CB,AD_CB,Ideal_CB,lr_CB,lda_CB,qda_CB,knn_CB,lasso_CB,GAM_CB,rf_CB,boost_CB,svm_CB,nn_CB))
colnames(table_CB) <- c("AF","AD","Ideal","LR","LDA","QDA","KNN","Lasso","GAM","RF","Boosting","SVM","Neural Network")
rownames(table_CB) <- c("Overall RMSSE")

#export table
 write.table(table_CB, file = "../Temporal-Aggregation/shiny app/ML_models/table_CB.txt", sep = "\t",row.names=TRUE)
