...
library(xgboost)
#library(httr)
#set_config(use_proxy("proxy.uns.ac.rs",8080))
...
# Ispitati dal radi ovaj boosting na 01 ili na Ta DIrect
```{r}
# require(gbm)
# train.data[,"marks"]<-(ifelse(train.data[,"marks"]=="TA",1,0))
# test.data[,"marks"]<-(ifelse(test.data[,"marks"]=="TA",1,0))
train.data01 <- train.data
test.data01 <- test.data
train.data01[,"marks"]<-(ifelse(train.data[,"marks"]=="TA",1,0))
test.data01[,"marks"]<-(ifelse(test.data[,"marks"]=="TA",1,0))
#Varijabla o kojoj se klasifikuje (marks) mora biti numeric 0 ili 1, ne faktor nikako!
```

# 6.1 Tree interaction depth=?
```{r}
source("R_sesions/cutoff_bost.R")
test.err=double(17)
for(n.depth in 1:17){
  fit.boost=gbm(marks~.,data=train.data01, distribution="bernoulli",n.trees=1000,shrinkage=0.01,interaction.depth=n.depth) #Distribution je bernuli jer je u pitanju klasifikacija
  
  pred=predict(fit.boost,test.data01,n.trees=1000,type="response")#ovdje treba response type jer ti daje onda brojeve oko 0 ili 1, bernulijevu raspodjelu, pa ti onda kasnije stavis >0.5
  ...
  cut <- cutoff_bost(pred)
   
  pred<-ifelse(pred>=cut,1,0)
  test.err[n.depth]=classificationMetrics(pred,test.data01[,"marks"])[c("err")]*100
  cat(n.depth, test.err[n.depth]," ")
}
...
for(shrinkage in range){
  i=i+1
  fit.boost=gbm(marks~.,data=train.data01,distribution="bernoulli",n.trees=1100,shrinkage=shrinkage,interaction.depth=5)
  pred=predict(fit.boost,test.data01,n.trees=1000,type="response");
  pred<-ifelse(pred>=0.6,1,0)
  miss_class.err[i]=classificationMetrics(pred,test.data01[,"marks"])[c("err")]*100
  cat(n.depth," ")
}
...
i=i+1
fit.boost=gbm(marks~.,data=train.data01,distribution="bernoulli",n.trees=n.trees[i],shrinkage=0.01,interaction.depth=10)
predmat=predict(fit.boost,newdata=test.data01,n.trees=n.trees[i],type="response")
cut <- cutoff_bost(predmat)
pred.boost<-ifelse(predmat>=cut,1,0)
berr[i]=(classificationMetrics(pred.boost,test.data01[,"marks"]))[c("err")]*100
cat(i,berr[i]," ")
...
# Best boosting tree
```{r}
#fit.best=gbm(marks~.,data=train.data01,distribution="bernoulli",n.trees=1100,shrinkage=0.01,interaction.depth=5)
#Mada je razlika bukvalno milimetarska kod ovog acc bude 0.634, a donjeg 0.636, ali zbog grafika iznda stavio sam kao donji...mada se prvi brže fituje.
fit.best=gbm(marks~.,data=train.data01,distribution="bernoulli",n.trees=4500,shrinkage=0.01,interaction.depth=10)
effect <- summary(fit.best)
plot(fit.best,i="trend")
predmat=predict(fit.best,newdata=test.data01,n.trees=1100,type="response")
boost.probs <- predmat
```
...
```{r,results='hide'}
source("cutoff_nn.R")
cut <- cutoff_nn(boost.probs)
```

```{r}
# boost.pred=ifelse(boost.probs>cut,1,0)
boost.pred=ifelse(boost.probs>cut,"TA","Direct")
roc.curve(test.data[,"marks"],boost.pred)
```

```{r Boosting evaluation}
(Boostingeval <-c(classificationMetrics(test.data[,"marks"],boost.pred,posClass = "TA"),AUC=roc.curve(test.data[,"marks"],boost.pred,FALSE)$auc,classificationMetrics(test.data[,"marks"],boost.pred,posClass = "TA","totU",cbM)))
```
...
summary(tune.out)
tune.out$best.model
#cost 1 gama 0.5 je najbolja kombinacija parametara
```

# Read tune out for SVM
```{r}
tune.out <- readRDS("tune_out.rds")
```


```{r}
svm.pred1 <- predict(tune.out$best.model,test.data)
#ovo daje najpreciznije prognoze, skoro kao RF ali ga nisam stavio da bi RF izgledao bolji.
```

```{r,eval=FALSE}
fit.svm=svm(as.factor(marks)~.,data=train.data,scale=FALSE,kernel="radial",cost=1,gama=0.5)
#Ovo je ubjedljivo najtacniji SVM model (tune.out$best)
svm.pred <- predict(fit.svm,test.data)
# dobićeš istu prognozu kao i da si radio sa onim tune.out$best, tj svm.pred1
```
...
cut <- cutoff_nn(as.vector(fitted2))
svm.pred <- ifelse(fitted2>cut,"TA","Direct")
#Interesanta je situacija da SVM kao i RF daje losije prognoze za isti model kada se trazi ovaj cut, kada direktno prognoziras svm.pred1 onda dobijas tacnije progoze.
```

```{r SVM evaluation}
# da si stavio svm.pred1 dobio bi tacnije prognoze skoro kao rf.
svm.pred <- svm.pred1
(SVMeval <- c(classificationMetrics(test.data[,"marks"],as.factor(svm.pred),posClass = "TA"),AUC=roc.curve(as.factor(test.data[,"marks"]),as.factor(svm.pred),FALSE)$auc,classificationMetrics(test.data[,"marks"],as.factor(svm.pred),posClass = "TA","totU",cbM)))
```
...
train.data01[,"marks"]<-(ifelse(train.data[,"marks"]=="TA",1,0))
test.data01[,"marks"]<-(ifelse(test.data[,"marks"]=="TA",1,0))
x_train = data.matrix(train.data01[,-43])                  # independent variables for train
y_train = train.data01[,43]                                # dependent variables for train
X_test = data.matrix(test.data01[,-43])                    # independent variables for test
y_test = test.data01[,43]    
xgboost_train = xgb.DMatrix(data=x_train, label=y_train)
xgboost_test = xgb.DMatrix(data=X_test, label=y_test)
```

# Best setup for XGBoost
```{r, eval=FALSE}
test.xg.err=double(10)
AUC.xg.err=double(10)
f.xg.err=double(10)
for(depth in 1:30){
  fit=xgboost(data = xgboost_train, max.depth=depth,nrounds=500,objective = "binary:logistic")
  pred_test = predict(fit, xgboost_test)
  cut_xg <- cutoff_nn(pred_test)
  xg.pred=ifelse(pred_test>cut_xg,"TA","Direct")
  test.xg.err[depth]=(classificationMetrics(xg.pred,test.data[,"marks"])["err"]*100)
  AUC.xg.err[depth]=roc.curve(as.factor(test.data[,"marks"]),as.factor(xg.pred),FALSE)$auc
  f.xg.err[depth]=(classificationMetrics(xg.pred,test.data[,"marks"])["F"])
  cat(depth," ")
}
matplot(1:depth,test.xg.err,pch=19,col=c("red"),type="b",ylab="Misclassification Error")
legend("topright",legend=c("Test"),pch=19,col=c("red"))#From the graph size=2
matplot(1:depth,AUC.xg.err,pch=19,col=c("red"),type="b",ylab="AUC")
legend("topright",legend=c("AUC"),pch=19,col=c("red"))#From the graph size=10
matplot(1:depth,f.xg.err,pch=19,col=c("red"),type="b",ylab="F Error")
legend("topright",legend=c("F"),pch=19,col=c("red"))#From the graph size=10
# The optimal setup is 23 variables
which.min(test.xg.err) #23 variables
```

# Best XG model
```{r}
xg <- xgboost(data = xgboost_train, max.depth=23,nrounds=500,objective = "binary:logistic")
summary(xg)
pred_test = predict(xg, xgboost_test)
cut_xg <- cutoff_nn(pred_test)
i<-cut_xg
xg.pred=ifelse(pred_test>i,"TA","Direct")
t<-table(xg.pred,ClassTable[-train,"marks"]);print(t)
(t[1,2]/(t[1,2]+t[2,2]))#FN
(t[2,1]/(t[1,1]+t[2,1]))#FP
1-mean(xg.pred==ClassTable[-train,"marks"])
```

# XG Boost eval
```{r xgeval}
xg.pred <- as.factor(xg.pred)
(XGeval <- c(classificationMetrics(test.data[,"marks"],xg.pred,posClass = "TA"),AUC=roc.curve(test.data[,"marks"],xg.pred,FALSE)$auc,classificationMetrics(test.data[,"marks"],xg.pred,posClass = "TA","totU",cbM)))
```
...
results
predTF1 <- predict(model, X_test)
# model %>% predict(X_test)

predTF1 <- as.numeric(predTF1)

cut_nn <- cutoff_nn(predTF1)
i<-cut_nn
predTF1=ifelse(predTF1>i,"TA","Direct")
t<-table(t.pred,ClassTable[-train,"marks"]);print(t)
(t[1,2]/(t[1,2]+t[2,2]))#FN
(t[2,1]/(t[1,1]+t[2,1]))#FP
1-mean(t.pred==ClassTable[-train,"marks"])

predTF1 <- as.factor(predTF1)
(TF1eval <- c(classificationMetrics(test.data[,"marks"],predTF1,posClass = "TA"),AUC=roc.curve(test.data[,"marks"],predTF1,FALSE)$auc,classificationMetrics(test.data[,"marks"],predTF1,posClass = "TA","totU",cbM)))
```

# Recurrent neural network
```{r}
# RNN
# Provjeri gore kod XG bosta da promjenis ovo sa velikim i malim x
# x_train <- x_train
# y_train 
# x_test <- X_test
# y_test
...
(RNNeval <- c(classificationMetrics(test.data[,"marks"],predRNN,posClass = "TA"),AUC=roc.curve(test.data[,"marks"],predRNN,FALSE)$auc,classificationMetrics(test.data[,"marks"],predRNN,posClass = "TA","totU",cbM)))
```
...
(CNNeval <- c(classificationMetrics(test.data[,"marks"],predCNN,posClass = "TA"),AUC=roc.curve(test.data[,"marks"],predCNN,FALSE)$auc,classificationMetrics(test.data[,"marks"],predCNN,posClass = "TA","totU",cbM)))
```
...
dtw.dist <- function(x, y) dtw(x, y, keep=TRUE)$distance

# Train the model using the k-NN algorithm with DTW distance
k <- 5 # number of nearest neighbors
trainLabels <- trainData$marks
testLabels <- testData$marks
#predLabels <- knn(train = trainData[, -5], test = testData[, -5], cl = trainLabels, k = k, prob = TRUE, use.all = TRUE, distance = dtw.dist)
predLabels <- knn(train = trainData[,-c(1,43)], test = testData[,-c(1,43)], cl = trainLabels, k = k, prob = TRUE, use.all = TRUE)


t <- table(predLabels,ClassTable[-train,"marks"]);print(t)
(t[1,2]/(t[1,2]+t[2,2]))#FN
(t[2,1]/(t[1,1]+t[2,1]))#FP
mean(predLabels==ClassTable[-train,"marks"])
roc.curve(ClassTable[-train,"marks"],predLabels)

(DTWeval <- c(classificationMetrics(ClassTable[-train,"marks"],predLabels,posClass = "TA"),AUC=roc.curve(ClassTable[-train,"marks"],predLabels,FALSE)$auc,classificationMetrics(ClassTable[-train,"marks"],predLabels,posClass = "TA","totU",cbM)))
...
# Evaluation table
```{r}
(EvalTable <- rbind(Nulleval,LReval,LDAeval,QDAeval,KNNeval,Lassoeval,GAMeval,RFeval,Boostingeval,SVMeval,FNNeval,Teval,XGeval,TF1eval,RNNeval,CNNeval, DTWeval))
```
...
svm.pred <- as.factor(svm.pred)
  ...
  class8 <- classificator(ideal_matrix,boost.pred,Ideal = TRUE)
  class9 <- classificator(ideal_matrix,svm.pred1,Ideal = TRUE)
  ...
  class13 <- classificator(ideal_matrix,xg.pred,Ideal = TRUE)
  class14 <- classificator(ideal_matrix,predTF1,Ideal = TRUE)
  class15 <- classificator(ideal_matrix,predRNN,Ideal = TRUE)
  class16 <- classificator(ideal_matrix,predCNN,Ideal = TRUE)
  class17 <- classificator(ideal_matrix,predLabels,Ideal = TRUE)
  
  table_all <- cbind(ideal_matrix[-train,1:3],class1[,4],class2[,4],class3[,4],class4[,4],class5[,4],class6[,4],class7[,4],class8[,4],class9[,4],class10[,4],class11[,4],class12[,4],class13[,4],class14[,4],class15[,4],class16[,4],class17[,4])
  colnames(table_all) <- c("AF","AD","Ideal","LR","LDA","QDA","KNN","Lasso","GAM","RF","Boosting","SVM","Null", "FNN", "DL_Torch","XG Boost","TensorFlow","RNN","CNN","DTW")
...
#High quality figure
```{r}
png("Fig_MCB_all3.png",  width = 300, height = 150, units = 'mm', res = 300)
colnames(table_all) <- c("AF","AD","Ideal","LR","LDA","QDA","KNN","Lasso","GAM","RF","Boosting","SVM","Null","FNN","DL Torch","XG Boost","TensorFlow","RNN","CNN","DTW")
...
boost_CB <- sum(table(boost.pred,ClassTable[-train,"marks"])*cbM)
...
xg_boost <- sum(table(xg.pred,ClassTable[-train,"marks"])*cbM)
tf_CB <- sum(table(predTF1,ClassTable[-train,"marks"])*cbM)
RNN_CB <- sum(table(predRNN,ClassTable[-train,"marks"])*cbM)
CNN_CB <- sum(table(predCNN,ClassTable[-train,"marks"])*cbM)
DTW_CB <- sum(table(predLabels,ClassTable[-train,"marks"])*cbM)

table_CB <- matrix(nrow = 1, c(AF_CB,AD_CB,Ideal_CB,lr_CB,lda_CB,qda_CB,knn_CB,lasso_CB,GAM_CB,rf_CB,boost_CB,svm_CB,nn_CB,nn_DL,xg_boost,tf_CB,RNN_CB,CNN_CB, DTW_CB))
colnames(table_CB) <- c("AF","AD","Ideal","LR","LDA","QDA","KNN","Lasso","GAM","RF","Boosting","SVM","FNN","DL Torch","XG Boost","TensorFlow","RNN","CNN","DTW")
...
* prebaciti sa USB-a radnu povrsinu
* iscitati do kraja knjigu i dodati razne modele
* pogledati sugestije recezenata i zavrsiti recenziju
* Dodati neke od modela koje je on predlozio u recenziji