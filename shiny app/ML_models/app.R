...
rownames(Tabela) <- c("max","min","LR","LDA","QDA","KNN","Lasso","GAM","RF","Boosting","SVM", "FNN", "DL_Torch","XG Boost","TensorFlow","RNN","CNN","DTW")
...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(width = 2,
                     ...
                     checkboxGroupInput(inputId="modeli", label="Select models:", choices=list("LR"=3,"LDA"=4,"QDA"=5,"KNN"=6,"Lasso"=7,"GAM"=8,"RF"=9,"Boosting"=10,"SVM"=11,"FNN"=12, "DL_Torch"=13,"XG Boost"=14,"TensorFlow"=15,"RNN"=16,"CNN"=17,"DTW"=18))
        ),
        mainPanel(
            tabsetPanel(
...
server <- function(input, output) {
    ...
    output$hist<-renderPlot(width = 1000, height = 1000,{
        ...
        if(length(input$modeli)!=0){
            ...
            legend("topright",legend=c(rownames(Tabela)[c(as.numeric((input$modeli)))]),pch=19,col=c(1,"slateblue2",3,4,5,6,"red",7,"orange2",1,"slateblue2",3,4,5,6,"red")[c(1:length(input$modeli))])
        }
    })
